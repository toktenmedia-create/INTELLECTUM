...
export async function responder({
  ...
  for (let vuelta = 0; vuelta < (VUELTAS[ambito] ?? 3); vuelta++) {
    ...
    if (mensaje.stop_reason === "refusal") {
      ...
      return { texto: MENSAJE_RECHAZO, lead: leadGuardado, acciones, leadTocado: contexto.leadTocado === true };
    }
  ...
  if (!textoFinal.trim()) {
    ...
    return { texto: MENSAJE_ATASCO, lead: leadGuardado, acciones, leadTocado: contexto.leadTocado === true };
  }
