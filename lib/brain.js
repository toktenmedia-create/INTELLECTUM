...
import { PLAN_POR_DEFECTO, claveDePlan, planDeRespaldo } from "./planes.js";
import { abrirAlmacen } from "./almacen.js";
import { recordarPersona } from "./memoria.js";
import { CLIENTE, comoEscribirnos } from "./cliente.js";
...
// Son funciones y no constantes porque la identidad se lee del entorno de cada
// copia: escrita como constante, un despliegue nuevo repartiría los datos de
// Intellectum al primer tropiezo.
const mensajeRechazo = () => {
  const vias = comoEscribirnos();
  return vias
    ? `Prefiero no responder eso por aquí. Si quieres, ${vias} y el equipo te ayuda.`
    : "Prefiero no responder eso por aquí. Si quieres, cuéntame en qué te puedo ayudar y seguimos.";
};
...
const mensajeAtasco = () => {
  const vias = comoEscribirnos();
  return vias
    ? `Perdona, se me complicó completar eso. ¿Lo intentamos otra vez? Si prefieres, ${vias} y lo coordina el equipo.`
    : "Perdona, se me complicó completar eso. ¿Lo intentamos otra vez?";
};
 ...
 * @param {object}   [opciones.demo]      un negocio de demostración (lib/demos.js).
 *   Si viene, manda sobre todo lo demás: el agente atiende con la identidad y
 *   la ficha de ese negocio inventado y SIN herramientas.
...
export async function responder({
  ...
  //
  // MODO DEMOSTRACIÓN. La portada deja que un visitante le escriba al agente de
  // un negocio que no existe (lib/demos.js). Es el MISMO cerebro y el mismo
  // prompt —esa es la gracia, no es una maqueta aparte que se desactualiza—,
  // con dos diferencias que no se negocian: la identidad y la ficha son las del
  // negocio de ejemplo, y no hay ni una herramienta. Sin herramientas no
  // agenda, no cotiza y no guarda un solo dato de quien apenas está mirando,
  // que es exactamente lo que uno quiere de una demostración.
  const dueño = demo ? null : await almacen.fichaDeCliente?.({ cliente: clienteId }).catch(() => null);
  // Si la ficha vino ILEGIBLE (la base no contestó) no se degrada al plan más
  // pequeño: eso apagaría agenda y cotizador justo durante la caída. Solo una
  // ficha que SÍ se leyó y no dice plan estrena con el mínimo.
  const planReal = claveDePlan(dueño?.plan ?? (dueño?.ilegible ? planDeRespaldo() : plan));

  const definiciones = demo ? [] : herramientasPara({ ambito, plan: planReal });
  ...
  const memoria =
    !demo && ambito === "publico" && mensajes.length <= 1
  ...
  const system = demo
    ...
    : ambito === "privado"
      ? construirSystemPrivado({ canal, duenoNombre, plan: planReal, ficha: dueño?.ficha })
      : construirSystem({
          canal,
          memoria,
          ficha: dueño?.ficha,
          // El prompt no puede mandar a llamar una herramienta que esta copia
          // no tiene: quedaría prometiendo una cotización que nunca llega.
          cotiza: definiciones.some((d) => d.name === "cotizar"),
        });
  ...
  for (let vuelta = 0; vuelta < (VUELTAS[ambito] ?? 3); vuelta++) {
    ...
    if (mensaje.stop_reason === "refusal") {
      const rechazo = mensajeRechazo();
      onTexto?.(rechazo);
      return { texto: rechazo, lead: leadGuardado, acciones, leadTocado: contexto.leadTocado === true };
    }
    ...
    if (llamadas.length === 0) {
      return { texto: textoFinal, lead: leadGuardado, acciones, leadTocado: contexto.leadTocado === true };
    }
  ...
  if (!textoFinal.trim()) {
    ...
    const atasco = mensajeAtasco();
    onTexto?.(atasco);
    return { texto: atasco, lead: leadGuardado, acciones, leadTocado: contexto.leadTocado === true };
  }
