...
import { PLAN_POR_DEFECTO } from "./planes.js";
...
const MENSAJE_RECHAZO =
  ...
  "o al WhatsApp +593 98 312 0003 y el equipo te ayuda.";
...
const MENSAJE_ATASCO =
  ...
  "escríbenos a info@intellectum.ec o al WhatsApp +593 98 312 0003 y lo coordina el equipo.";
...
export async function responder({
  ...
  const system =
    ambito === "privado"
      ? construirSystemPrivado({ canal, duenoNombre, plan })
      : construirSystem({ canal });

  const definiciones = herramientasPara({ ambito, plan });
  const mensajes = historial.map((m) => ({ role: m.role, content: m.content }));

  const almacen = abrirAlmacenSeguro();
  const contexto = { almacen, cliente: clienteId, canal, ambito, plan, meta, modelo: MODELO };
  ...
  for (let vuelta = 0; vuelta < (VUELTAS[ambito] ?? 3); vuelta++) {
    const mensaje = await unaVuelta({ system, mensajes, definiciones, onTexto });
