...
const SITIO = () => (process.env.SITIO_URL || "https://www.intellectum.ec").replace(/\/+$/, "");
...
const TITULOS = {
  nueva: "Tu consultoría quedó agendada",
  movida: "Tu consultoría cambió de hora",
  cancelada: "Tu consultoría quedó cancelada",
  reagendar: "Tuvimos que cancelar tu consultoría",
};
...
${boton(`${SITIO()}/chat?intencion=reagendar`, "Elegir otra hora")}
</td></tr>`;
  } else if (tipo !== "cancelada") {
    // El enlace lleva la referencia dentro, así que la persona no tiene que
    // entender ni teclear nada: llega al chat y IntelliA ya sabe qué cita es.
    const url = ref
...
¿Necesitas moverla o cancelarla? IntelliA lo resuelve al momento.
</div>
${boton(url, "Mover o cancelar mi cita")}
...
<span style="font-family:${SANS};font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:${C.tenue};">Intellectum AI Solutions</span>
</td></tr>
...
<a href="mailto:info@intellectum.ec" style="color:${C.dim};text-decoration:none;">info@intellectum.ec</a> &nbsp;·&nbsp;
<a href="https://wa.me/593967518060" style="color:${C.dim};text-decoration:none;">+593 96 751 8060</a> &nbsp;·&nbsp;
<a href="${SITIO()}" style="color:${C.dim};text-decoration:none;">intellectum.ec</a>
</td></tr>
...
export function textoDeCita({ nombre = "", cuando = "", codigo = "", cambio = "nueva" } = {}) {
  ...
  if (tipo === "cancelada" || tipo === "reagendar") {
    ...
    if (tipo === "reagendar") {
      l.push(``, `Elige tu nueva hora aquí:`, `  ${SITIO()}/chat?intencion=reagendar`);
    }
  } else {
    l.push(
      `${cuando} (hora de Ecuador).`,
      `Dura 30 minutos, sin costo y sin compromiso.`,
      ``,
      tipo === "movida"
        ? `La invitación adjunta reemplaza a la anterior.`
        : `Adjunto va el archivo para añadirla a tu calendario.`,
      ``,
      `¿Necesitas moverla o cancelarla? Escríbele a IntelliA aquí y lo resuelve al momento:`,
  ...
  l.push(
    ...
    `info@intellectum.ec · ${SITIO().replace(/^https?:\/\//, "")}`,
  );
  return l.join("\n");
}
