...
export function elegirParaSeguimiento({
  ...
  // Y cuáles tomó el equipo a mano: "el bot se calla" incluye los mensajes que
  // el bot INICIA — una plantilla de seguimiento en medio de una negociación
  // humana es el bot hablando donde prometió silencio.
  const manosHumanas = new Set();
  for (const c of conversaciones) {
    const fecha = c.actualizado_en ?? c.creado_en;
    if (c.sesion && fecha) ultimaSeñal.set(`${c.canal}|${c.sesion}`, fecha);
    if (c.sesion && c.modo === "humano") manosHumanas.add(`${c.canal}|${c.sesion}`);
  }
  ...
  // PRIMERO los que la persona misma pidió. No pasan por la regla de los tres
  // días de silencio —esa regla existe para no molestar a quien no pidió nada—,
  // pero sí por todo lo demás: la baja, el "ya se le escribió" y que el lead
  // siga sin atender. Que alguien haya dado permiso no lo obliga a recibirlo
  // si mientras tanto pidió salir.
  for (const p of programados) {
    const lead = porId.get(p.lead_id);
    if (!lead || !p.sesion) { descartes.programado_ya_atendido++; continue; }
    if ((lead.estado ?? "nuevo") !== "nuevo") { descartes.ya_atendido++; continue; }
    if (bajas.has(p.sesion)) { descartes.dio_de_baja++; continue; }
    if (manosHumanas.has(`whatsapp|${p.sesion}`)) { descartes.en_manos_humanas++; continue; }
    if (yaSeguidos.has(lead.id) || yaEnLista.has(lead.id)) { descartes.ya_se_le_escribio++; continue; }

    yaEnLista.add(lead.id);
    elegidos.push({
      lead_id: lead.id,
      numero: p.sesion,
      nombre: limpiarParametro(lead.nombre, "buen día"),
      concepto: limpiarParametro(conceptoCotizado(lead), "el servicio que consultaste"),
      dias_de_silencio: Math.max(0, Math.floor((ahora - new Date(lead.creado_en)) / 86_400_000)) || 0,
      lo_pidio: true,
    });
  }

  for (const lead of leads) {
    if (yaEnLista.has(lead.id)) continue;
    ...
    if (manosHumanas.has(`whatsapp|${lead.sesion}`)) { descartes.en_manos_humanas++; continue; }
  ...
  // Los que lo pidieron van primero pase lo que pase: si el tope corta, no
  // puede cortar una promesa hecha a alguien. Dentro de cada grupo, primero
  // quien lleva más tiempo esperando.
  elegidos.sort(
    (a, b) => Number(Boolean(b.lo_pidio)) - Number(Boolean(a.lo_pidio)) || b.dias_de_silencio - a.dias_de_silencio,
  );
