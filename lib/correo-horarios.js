...
export function correoDeHorarios({ nombre = "", horarios = [], intro = "", motivo = "" }) {
  ...
  const texto =
    intro ||
    "Te dejamos las horas libres para tu consultoría gratuita de 30 minutos. Toca la que te sirva y queda agendada al instante.";
...
Son 30 minutos, sin costo y sin compromiso.<br />
...
export function textoDeHorarios({ nombre = "", horarios = [], intro = "" }) {
  const lineas = [
    ...
    ``,
    intro || "Estas son las horas libres para tu consultoría gratuita de 30 minutos:",
    ``,
  ];
  for (const h of horarios) {
  ...
  lineas.push(`Son 30 minutos, sin costo y sin compromiso.`);
  lineas.push(`Intellectum AI Solutions · info@intellectum.ec · +593 96 751 8060`);
  return lineas.join("\n");
}
