...
Si la persona claramente NO es un prospecto (${comercial(n, casa).noProspecto}), sé amable, respóndele
 ...
 */
function fichaEnUso(ficha, casa = esIntellectum()) {
  const propia = String(ficha ?? "").trim();
  if (propia) return propia;
  if (casa) return FICHA;
