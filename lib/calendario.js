...
export async function moverCita(cita, nuevoInicioISO) {
  ...
  // El mismo candado optimista de crearCita, por la otra puerta: mover también
  // es escribir en una franja disputada. Si tras el PATCH aparece un competidor
  // que gana por antigüedad (el desempate universal de este archivo), la cita
  // REGRESA a su hora original — nadie pierde su reserva por un empate.
  // OJO: un evento movido conserva su `created` original, así que casi siempre
  // gana; este retroceso cubre el caso de dos reagendamientos simultáneos.
  try {
    const evento = await respuesta.json().catch(() => null);
    const creadoNuestro = `${evento?.created ?? ""}|${cita.id}`;
    const competidores = await competidoresEnFranja(slot, cita.id);
    if (competidores.some((e) => `${e.created ?? ""}|${e.id}` < creadoNuestro)) {
      await fetch(
        `${API}/calendars/${encodeURIComponent(calendarioCitas())}/events/${encodeURIComponent(cita.id)}`,
        {
          method: "PATCH",
          ...
          body: JSON.stringify({
            start: { dateTime: cita.inicio, timeZone: ZONA },
            end: { dateTime: cita.fin, timeZone: ZONA },
          }),
        },
      );
      return { ok: false, motivo: "ocupado", etiqueta: etiquetaHumana(slot.inicio) };
    ...
    // Si la comprobación falla, el movimiento queda: mejor un choque rarísimo
    // que deshacerle la cita a alguien por un tropiezo de red.
    console.error("[CITA] no se pudo comprobar el choque al mover:", err?.message ?? err);
  }
...
export async function crearCita({ inicioISO, nombre, contacto, telefono, empresa, motivo }) {
  ...
  const cuerpo = {
    summary: `${mayuscula(NEGOCIO.evento)} — ${nombre || "sin nombre"}${empresa ? ` (${empresa})` : ""}`,
    description: [
      `Agendada por ${NEGOCIO.agente} desde ${NEGOCIO.dominio}.`,
      ``,
  ...
  try {
    if (await laGanoOtro(evento, slot)) {
      await cancelarCita(evento.id).catch(() =>
        // Un solo reintento: si tampoco sale, queda gritado con el id para
        // retirarlo a mano — un evento zombi es una doble reserva con datos.
        cancelarCita(evento.id).catch((err) =>
          console.error(
            `[CITA] choque detectado pero no se pudo retirar la copia ${evento.id}:`,
            err?.message,
          ),
        ),
      );
...
async function laGanoOtro(nuestro, slot) {
  const competidores = await competidoresEnFranja(slot, nuestro.id);
  const creadoNuestro = `${nuestro.created ?? ""}|${nuestro.id}`;
  return competidores.some((e) => `${e.created ?? ""}|${e.id}` < creadoNuestro);
}

/**
 * Los eventos que de verdad disputan una franja: con hora (no de día entero),
 * vivos, ajenos, y OPACOS. Los marcados "transparent" (Disponible) se
 * descartan porque freeBusy tampoco los cuenta: si aquí contaran, un bloque
 * "horario de atención" antiguo le ganaría a toda cita nueva para siempre.
 */
async function competidoresEnFranja(slot, idPropio) {
  const url =
    `${API}/calendars/${encodeURIComponent(calendarioCitas())}/events` +
...
export function invitacionICS({ inicioISO, finISO, nombre, id, secuencia = 0, cancelada = false }) {
  ...
  return [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    `PRODID:-//${escaparICS(NEGOCIO.nombre)}//${escaparICS(NEGOCIO.agente)}//ES`,
    "CALSCALE:GREGORIAN",
    `METHOD:${cancelada ? "CANCEL" : "REQUEST"}`,
    "BEGIN:VEVENT",
    `UID:${id || Date.now()}@${NEGOCIO.dominio.replace(/^www\./, "") || NEGOCIO.slug}`,
    `SEQUENCE:${secuencia}`,
    `DTSTAMP:${sello(new Date().toISOString())}`,
    `DTSTART:${sello(inicioISO)}`,
    `DTEND:${sello(finISO)}`,
    `SUMMARY:${escaparICS(NEGOCIO.citaTitulo)} — ${escaparICS(NEGOCIO.nombre)}`,
    `DESCRIPTION:${escaparICS(mayuscula(NEGOCIO.evento))} de ${REGLAS_AGENDA.duracionMinutos} minutos con el equipo de ${escaparICS(NEGOCIO.nombreCorto)}.${nombre ? ` Reservada por ${escaparICS(nombre)}.` : ""}`,
    // Sin correo no hay ORGANIZER: "mailto:" a secas es una línea inválida y
    // hay calendarios que rechazan el archivo entero por ella.
    ...(NEGOCIO.correo ? [`ORGANIZER;CN=${escaparICS(NEGOCIO.nombre)}:mailto:${NEGOCIO.correo}`] : []),
    ...
    ...(cancelada
      ...
      : [
          ...
          "TRIGGER:-PT30M",
          "ACTION:DISPLAY",
          `DESCRIPTION:Tu ${escaparICS(NEGOCIO.evento)} con ${escaparICS(NEGOCIO.nombreCorto)} empieza en 30 minutos`,
          "END:VALARM",
        ]),
    "END:VEVENT",
    "END:VCALENDAR",
...
/** "consultoría" → "Consultoría". Para empezar una frase con el nombre suyo. */
const mayuscula = (t) => {
