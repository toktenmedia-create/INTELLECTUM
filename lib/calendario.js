 ...
 */
export async function horariosLibres({ maximo = REGLAS_AGENDA.maximoOpciones } = {}) {
  const candidatos = candidatosDeAgenda();
  if (candidatos.length === 0) return [];

  const ocupados = await franjasOcupadas(
    candidatos[0].inicio,
    candidatos[candidatos.length - 1].fin,
  );

  const libres = candidatos.filter((slot) => !chocaCon(slot, ocupados));
  return libres.slice(0, maximo).map((slot) => ({
    inicio: slot.inicio,
    fin: slot.fin,
...
/**
 * ¿Esa hora es una de las que la agenda puede ofrecer?
 *
 * El modelo tiene que devolver una hora exacta de las que se le mostraron, y a
 * veces se inventa uno cercano ("las 8" cuando se abre a las 9). Esto lo corta
 * antes de escribir nada en el calendario, y sin depender de recordar qué se
 * ofreció en el mensaje anterior.
 */
export function esSlotOfrecible(iso) {
  const objetivo = new Date(iso).getTime();
  if (Number.isNaN(objetivo)) return false;
  return candidatosDeAgenda().some((s) => new Date(s.inicio).getTime() === objetivo);
}
 ...
 */
export async function crearCita({ inicioISO, nombre, contacto, empresa, motivo }) {
 ...
 */
export function invitacionICS({ inicioISO, finISO, nombre, id }) {
