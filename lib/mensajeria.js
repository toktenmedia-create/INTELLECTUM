...
/**
 * POST crudo al Graph. Devuelve { ok, detalle, wamid } y nunca lanza.
 *
 * Meta acepta el mensaje y devuelve su identificador (wamid); si más tarde no
 * lo puede entregar, lo cuenta por el webhook con ese mismo wamid en
 * value.statuses. Por eso se guarda con la entrega: sin él, un "failed" de
 * Meta no se puede cruzar con lo que se mandó.
 *
 * Un fallo aquí ya no es solo una línea de registro: queda como evento
 * mensaje_fallido y le suena al operador (lib/alertas.js). Es la lección del
 * número que estuvo restringido cuatro meses sin que nadie lo viera.
 */
async function mandar(cuerpo, etiqueta, bitacora = null, anotacion = null) {
  try {
    ...
    if (!respuesta.ok) {
      ...
      await anotarFallo(bitacora, { ...anotacion, etiqueta, estado: respuesta.status, detalle });
    ...
    try {
      ...
      /* sin cuerpo legible; el envío igual salió */
    }
    // Solo lo entregado se cuenta: lo que Meta rechazó no se cobra.
    if (anotacion) await anotarEntrega(bitacora, { ...anotacion, wamid });
    return { ok: true, wamid };
  } catch (err) {
    const detalle = String(err?.message ?? err);
    console.error(`[WHATSAPP] ${etiqueta} no salió:`, detalle);
    await anotarFallo(bitacora, { ...anotacion, etiqueta, detalle });
    return { ok: false, detalle };
  }
}

/**
 * Deja constancia de un envío que Meta rechazó y le avisa al operador.
 *
 * Salvo cuando el destinatario ES el equipo: ese aviso lo manda la propia
 * alarma, y alertar de que la alarma falló volvería a llamar a la alarma.
 * Nunca lanza.
 */
async function anotarFallo(bitacora, detalle) {
  if (bitacora?.almacen) {
    try {
      await bitacora.almacen.registrarEvento({
      ...
      console.error("[WHATSAPP] no se pudo anotar el fallo:", err?.message ?? err);
    }
  }
  if (detalle?.destino === "equipo") return;
  try {
    // Importación tardía: lib/alertas.js usa este archivo para mandar el
    // WhatsApp del aviso, y un import fijo en los dos sentidos se enreda.
    const { alertarAlOperador } = await import("./alertas.js");
    await alertarAlOperador({
      asunto: "WhatsApp no pudo enviar un mensaje",
      cuerpo:
        `${detalle.etiqueta}: ${detalle.detalle ?? "sin detalle"}` +
        (detalle.estado ? ` (HTTP ${detalle.estado})` : "") +
        ". Si se repite, revisa el número en Meta (token vencido, calidad, límite de mensajes) o consulta /api/salud.",
      clave: "meta_envio",
      almacen: bitacora?.almacen ?? null,
      detalle: { motivo: detalle.motivo ?? null, estado: detalle.estado ?? null },
    });
  } catch (err) {
    console.error("[WHATSAPP] no se pudo alertar al operador:", err?.message ?? err);
  }
}
