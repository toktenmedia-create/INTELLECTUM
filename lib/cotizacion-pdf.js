...
/** Los datos fijos de la empresa. Si cambian, cambian aquí y en lib/ficha.js. */
const EMPRESA = {
  nombre: "Intellectum AI Solutions S.A.S.",
  ruc: "RUC 1793236353001",
  ciudad: "Quito, Ecuador",
  correo: "info@intellectum.ec",
  whatsapp: "+593 96 751 8060",
  web: "www.intellectum.ec",
};
...
export async function construirCotizacionPDF({ cotizacion, persona = {}, fecha = new Date(), referencia = "" }) {
  ...
  const logo = await doc.embedPng(fs.readFileSync(path.join(RAIZ, "logo.png")));

  doc.setTitle(`Cotización Intellectum${referencia ? ` ${referencia}` : ""}`);
  doc.setAuthor(EMPRESA.nombre);
  doc.setCreator(EMPRESA.web);
  ...
  escribir("El número exacto sale del diagnóstico gratuito de 30 minutos.", MARGEN + 20, Y_CTA + ALTO_CTA - 23, {
    fuente: fuerte, tam: 10.5, color: TINTA.texto,
  });
  escribir(`Agenda el tuyo en ${EMPRESA.web}/chat o responde este mensaje.`, MARGEN + 20, Y_CTA + 17, {
  ...
  const contacto = `${EMPRESA.correo}   ·   ${EMPRESA.whatsapp}   ·   ${EMPRESA.web}`;
  escribir(contacto, MARGEN, PIE - 26, { fuente: fuerte, tam: 9.5, color: TINTA.claroSobreOscuro });
  escribir(`${EMPRESA.nombre}   ·   ${EMPRESA.ruc}   ·   ${EMPRESA.ciudad}`, MARGEN, PIE - 44, {
