...
import { CLIENTE, NEGOCIO, comoEscribirnos, esIntellectum, datosQueFaltan } from "./cliente.js";
...
export function calcularCotizacion({
  ...
  // La voz solo se suma si el módulo está disponible. En lista de espera se
  // anota el interés y se dice de frente: cotizar algo que no se puede
  // entregar es la cifra falsa más cara que existe.
  const vozEnEspera = si(quiere_llamadas) && PRECIOS.modulo_voz.disponible === false;
  const quiereVoz = si(quiere_llamadas) && !vozEnEspera;
  const quiereWhatsapp = si(quiere_whatsapp);
  const notaVoz = vozEnEspera
    ? " OJO: el módulo de voz (llamadas) está EN LISTA DE ESPERA y no se cotizó: dile de " +
      "frente que las llamadas se activan más adelante, que su interés quedó anotado, y " +
      "que esta cifra cubre solo WhatsApp y web. No prometas fecha."
    : "";
...
export const HERRAMIENTAS = {
  ...
  guardar_lead: {
      ...
      input_schema: {
        ...
        properties: {
          ...
          sector: {
            type: "string",
            description:
              "Si escribe en nombre de una empresa, a qué se dedica (clínica, retail, " +
              "inmobiliaria...). Si es una persona particular, vacío.",
          },
          cargo: {
            type: "string",
            description:
              "Rol de la persona dentro de su empresa (dueño, gerente, operaciones...), solo " +
              "si escribe por una empresa. Si es cliente particular, vacío.",
          },
          necesidad: {
            type: "string",
          ...
          tamano_empresa: {
            type: "string",
            description:
              "Tamaño aproximado del equipo o del volumen que maneja, solo si lo mencionó y " +
              "solo si aplica al negocio. Vacío en caso contrario.",
          },
          urgencia: {
            type: "string",
    ...
    async ejecutar(entradaCruda, ctx) {
      ...
      // La señal honesta para el calificador: hubo ficha escrita de verdad.
      // (Un rebote de la herramienta no la enciende; ver lib/calificar.js.)
      if (guardado) ctx.leadTocado = true;

      // Si el almacén FUSIONÓ (la persona ya tenía ficha reciente), no se
      // grita "lead nuevo" al equipo: ya lo conocían. La ficha quedó completada
      // y eso es todo lo que había que hacer.
      if (guardado?.fue_fusion) {
        return (
          `La persona ya estaba registrada (ficha ${guardado.id}, completada con lo nuevo). ` +
          `No anuncies "te registré": continúa la conversación con naturalidad.`
        );
      }
  ...
  cotizar: {
      ...
      input_schema: {
        ...
        properties: {
          ...
          quiere_llamadas: {
            type: "boolean",
            description:
              "true si además quiere que un agente conteste llamadas de voz. Ese módulo está " +
              "en lista de espera: se anota el interés, pero NO se cotiza ni se promete fecha.",
          },
          integraciones: {
            type: "number",
            description:
    ...
    async ejecutar(entradaCruda, ctx) {
      ...
      if (existente) {
        ctx.leadTocado = true; // la vuelta trabaja sobre la ficha existente
        ...
        return (
          ...
          `que el número exacto sale del diagnóstico en la ${NEGOCIO.cita}. ` +
          "Cierra ofreciendo agendarla. La persona ya estaba registrada: no " +
      ...
      // El correo de "lead nuevo" solo si de verdad es nuevo: en una fusión el
      // equipo ya conocía a la persona (y la cotización viaja en su evento).
      if (!guardado?.fue_fusion) {
      ...
      return (
        ...
        "Preséntala así: di el rango TAL CUAL, aclara que es referencial y sin IVA, y " +
        `que el número exacto sale del diagnóstico en la ${NEGOCIO.cita}. ` +
        "Cierra ofreciendo agendarla. La persona ya quedó registrada: no " +
        "llames a guardar_lead ni prometas descuentos."
      );
    },
  },
  ...
  escalar_a_humano: {
    ...
    async ejecutar(entrada, ctx) {
      ...
      // En WhatsApp, escalar también entrega el volante: el bot se calla y la
      // conversación queda en manos del equipo, que responde desde el panel.
      // En la web no hay traspaso: cuando el visitante cierra la página no hay
      // forma de empujarle la respuesta de una persona.
      let traspasada = false;
      if (ctx.canal === "whatsapp" && ctx.meta?.sesion) {
        const sesion = String(ctx.meta.sesion);
        try {
          // crear:true — el traspaso puede llegar antes del primer guardado
          // del hilo (la persona pide humano en su primer mensaje) y ahí la
          // fila de la conversación aún no existe.
          await ctx.almacen.cambiarModo?.({
            cliente: ctx.cliente,
            canal: "whatsapp",
            sesion,
            modo: "humano",
          ...
          await ctx.almacen
            .registrarEvento({
              tipo: "traspaso_humano",
              actor: "agente",
              cliente: ctx.cliente,
              detalle: { canal: "whatsapp", sesion, por: "escalar_a_humano", motivo },
            })
            .catch(() => {});
        } catch (err) {
          console.error("[ESCALAR] no se pudo pasar a manos humanas:", err?.message ?? err);
        }
      }

      if (traspasada) {
        return enviado.entregado
          ? "El equipo ya fue avisado y esta conversación queda en manos de una persona: dile que desde ahora le responde alguien del equipo por este mismo chat, sin prometer un plazo concreto. Tú ya no respondes más aquí."
          : "La conversación queda en manos de una persona del equipo, que le responderá por este mismo chat." +
            (NEGOCIO.correo ? ` Dale también el correo ${NEGOCIO.correo} por si prefiere escribir directo.` : "") +
            " Tú ya no respondes más aquí.";
      }
      return enviado.entregado
        ? "El equipo ya fue avisado. Dile a la persona que alguien la contactará, sin prometer un plazo concreto."
        : comoEscribirnos()
          ? `No se pudo avisar automáticamente. Dile que ${comoEscribirnos()} para que hable directo con el equipo.`
          : "No se pudo avisar automáticamente y esta copia no tiene un correo ni un teléfono configurados. Discúlpate, dile que el equipo la contactará y no inventes ninguna vía de contacto.";
    },
  },
  ...
  revisar_ficha: {
    ...
    async ejecutar(_entrada, ctx) {
      // La ficha de ESTA copia, no la de casa: en el panel de la ferretería,
      // "qué falta configurar" tiene que hablar de la ferretería.
      const dueño = await ctx?.almacen?.fichaDeCliente?.({ cliente: ctx.cliente }).catch(() => null);
      const ficha = String(dueño?.ficha ?? "").trim() || (esIntellectum() ? FICHA : "");
      if (!ficha) {
        return JSON.stringify({
          campos_pendientes: ["la ficha entera"],
          cuantos_pendientes: 1,
          servicios_en_catalogo: 0,
          nota: "Este negocio todavía no tiene ficha cargada: el agente atiende sin catálogo.",
        });
      }
  ...
  estado_del_sistema: {
    ...
    async ejecutar(_entrada, ctx) {
      ...
      // Lo que le falta a ESTA copia para hablar en nombre de su negocio. Sin
      // esto el reporte salía todo en verde mientras el agente atendía sin
      // poder decir un correo ni un WhatsApp, y el dueño no tenía cómo
      // enterarse: el aviso de arranque solo se ve en el registro de Vercel.
      ...
      return JSON.stringify({
        negocio: {
          ...
          datos_de_contacto_sin_poner: faltan.length ? faltan : null,
          advertencia: faltan.length
            ? "Con esas variables vacías el agente NO reparte correo, WhatsApp ni dirección web: " +
              "calla el dato en vez de dar uno ajeno. Se ponen en las variables del despliegue."
            : null,
        },
        cerebro: {
          modelo: ctx.modelo,
        ...
        canales: {
          ...
          // La agenda se apaga sola sin credenciales de Google, y cuando eso
          // pasa la página /agenda responde 503 y manda a escribir por
          // WhatsApp. Antes no se reportaba y parecía que todo estaba bien.
          agenda: agendaConfigurada()
            ? "conectada a Google Calendar"
            : "dormida (faltan las credenciales de Google)",
        },
        salida_de_leads: {
  ...
  ver_disponibilidad: {
    ...
    definicion: {
      ...
      description:
        `Consulta las próximas horas libres reales para la ${NEGOCIO.cita}. ` +
  ...
  agendar_cita: {
    ...
    definicion: {
      name: "agendar_cita",
      description:
        `Reserva la ${NEGOCIO.cita} en la agenda real y le envía a la persona ` +
        "la confirmación con la invitación para su calendario. Llámala SOLO cuando la persona ya " +
        "eligió una de las horas que te devolvió ver_disponibilidad y ya te dio su nombre y un " +
        "contacto (su correo o, si conversan por WhatsApp y no quiere dar correo, su número).",
      strict: true,
      input_schema: {
        type: "object",
        properties: {
          ...
          contacto: {
            type: "string",
            description:
              "Su correo electrónico, para mandarle la confirmación con la invitación. Si la " +
              "conversación es por WhatsApp y no quiere dar correo, manda su número de WhatsApp: " +
              "la confirmación se la das tú en el chat y el recordatorio le llega por WhatsApp.",
          },
    ...
    async ejecutar(entrada, ctx) {
      ...
      const resultado = await agendarConsultoria({
        ...
        canal: ctx.canal,
        meta: ctx.meta,
        origen: ctx.canal === "whatsapp" ? `${NEGOCIO.agente} por WhatsApp` : `${NEGOCIO.agente} por el chat web`,
      });
  ...
  enviar_documento: {
    ...
    definicion: {
      ...
      description:
        "Manda a la persona SU cotización en PDF, con la marca del negocio y los " +
    ...
    async ejecutar(_entrada, ctx) {
      ...
      if (!cotizacion) {
        return (
          ...
          `PDF que mandar. Ofrécele la ${NEGOCIO.cita} en su lugar.`
        );
      }
      ...
      if (ctx.canal === "whatsapp") {
        const enlace = enlaceDeCotizacion(lead.id);
        if (!enlace) {
          return (
            "Esta copia no tiene dominio configurado, así que no se puede mandar el PDF " +
            "por WhatsApp. Pídele un correo y ofrécele mandárselo por ahí."
          );
        }
        ...
        return entregado
          ? `Cotización enviada por WhatsApp (referencia ${referencia}). Dilo con naturalidad y ofrece agendar la ${NEGOCIO.cita}.`
...
export function herramientasPara({ ambito = "publico", plan } = {}) {
  ...
  return permitidas
    .filter((nombre) => nombre !== "cotizar" || puedeCotizar())
...
/**
 * ¿Puede ESTA copia dar precios?
 *
 * La matriz de precios vive en lib/precios.js, o sea en el código, y es la de
 * Intellectum. Una copia ajena que llame a cotizar le entrega al cliente de
 * una ferretería las tarifas de una empresa de software: no es un texto feo,
 * es una cifra falsa dicha con seguridad, y encima queda por escrito en un PDF
 * y en un correo. Mientras los precios no viajen en la ficha de cada cliente,
 * la copia ajena no cotiza: recoge el contacto y el equipo cotiza a mano.
 */
let avisoCotizarDado = false;
function puedeCotizar() {
  if (esIntellectum()) return true;
  if (!avisoCotizarDado) {
    avisoCotizarDado = true;
    console.warn(
      `[HERRAMIENTAS] la copia de "${CLIENTE}" no cotiza: los precios de lib/precios.js son los de Intellectum. ` +
        "El agente pedirá los datos y dejará el lead para que el equipo cotice.",
    );
  }
  return false;
}
...
export function buscarHerramienta(nombre, ambito) {
  ...
  // Que no se ofrezca no basta: el modelo puede inventarse la llamada, y esta
  // es la puerta por la que se ejecuta de verdad.
  if (nombre === "cotizar" && !puedeCotizar()) return null;
...
export function loQueSeEntregaHoy() {
  ...
  for (const [clave, plan] of Object.entries(PLANES)) {
    ...
    for (const nombre of plan.herramientas) {
      ...
      // El cotizador solo existe para la copia de casa mientras la matriz de
      // precios viva en el código: en la propuesta de un cliente cuenta como
      // pendiente, no como entregado.
      const construida = h.estado === "listo" || h.estado === "automatica";
      if (construida && (nombre !== "cotizar" || puedeCotizar())) listas.push(linea);
