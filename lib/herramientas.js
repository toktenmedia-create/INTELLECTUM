 ...
 *
 * Reglas de negocio: la voz nunca va sola (se monta sobre el Operativo);
 * más de 40 consultas diarias suben un nivel de plan, porque el tope de
 ...
 * WhatsApp o voz cotiza web MÁS plan de agente, porque el Esencial incluido
 * en la web no cubre esos canales. El valor_estimado del lead es la
 * implementación mínima más seis mensualidades: una vara conservadora y
 * uniforme para comparar oportunidades en el embudo, no una promesa.
 */
export function calcularCotizacion({
  ...
  // Agente Esencial adentro. Si además pidió WhatsApp o voz, se sigue de
  // largo: la web se suma al plan de agente más abajo.
  if (proyecto && !quiereWhatsapp && !quiereVoz) {
    ...
    return {
      ...
      detalle:
        ...
        "Incluye el Agente Esencial integrado con su primer mes de servicio: la web " +
        "nace atendiendo sola.",
    };
  }

  // Plan de agente según lo que pidió; el volumen alto sube un nivel.
  let clave =
    objetivo === "procesos_internos"
      ? "plataforma"
      : quiereWhatsapp || objetivo === "citas_y_agenda" || quiereVoz
        ? "operativo"
        : "esencial";
  if (volumen_diario === "mas_de_40" && clave !== "plataforma") {
    clave = clave === "esencial" ? "operativo" : "plataforma";
  }
...
export const HERRAMIENTAS = {
  ...
  cotizar: {
      ...
      input_schema: {
        ...
        properties: {
          ...
          integraciones: {
            type: "number",
            description:
              "Cuántos sistemas habría que conectar (ERP, hoja de cálculo, agenda distinta " +
              "de Google, pasarela...). 0 si no mencionó ninguno; nómbralos en detalle.",
          },
          volumen_diario: {
            type: "string",
    ...
    async ejecutar(entradaCruda, ctx) {
      ...
      let existente = null;
      try {
        existente = await ctx.almacen.leadDeSesion?.({
          cliente: ctx.cliente,
          canal: ctx.canal,
      ...
      if (existente) {
        await ctx.almacen
          .actualizarLead({
            ...
            nota: `Recotizó por chat: ${cotizacion.concepto}. Rango dado: ${cotizacion.resumen}.`,
          })
          .catch((err) =>
      ...
      const lead = {
        ...
        resumen: `Cotizó por chat: ${cotizacion.concepto}. Rango dado: ${cotizacion.resumen}.`,
        valor_estimado: cotizacion.valor_estimado,
      };
      let guardado = null;
      try {
