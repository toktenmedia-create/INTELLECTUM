...
export const HERRAMIENTAS = {
  ...
  ver_disponibilidad: {
    ...
    async ejecutar(_entrada, ctx) {
      ...
      return (
        "Horas libres (la persona debe elegir una; pásale a agendar_cita el valor exacto de 'inicio'):\n" +
        libres.map((h) => `- ${h.etiqueta}  →  inicio: ${h.inicio}`).join("\n") +
        "\nOfrécele dos o tres, no la lista completa. Hora de Ecuador."
      );
    },
  },
  ...
  agendar_cita: {
    ...
    definicion: {
      ...
      description:
        ...
        "correo. El campo 'inicio' debe ser exactamente uno de los que te dio ver_disponibilidad.",
      strict: true,
      input_schema: {
        type: "object",
        properties: {
          inicio: {
            type: "string",
            description:
              "La hora elegida, copiada tal cual del resultado de ver_disponibilidad. No la reescribas.",
          },
        ...
        required: ["inicio", "nombre", "contacto", "empresa", "motivo"],
        additionalProperties: false,
      },
    },
    async ejecutar(entrada, ctx) {
      if (!esSlotOfrecible(entrada.inicio)) {
        return (
          "Esa hora no es una de las disponibles. NO la agendes ni se la ofrezcas a la persona. " +
          "Vuelve a llamar a ver_disponibilidad y ofrécele únicamente las horas que te devuelva."
        );
      }

      const cita = await crearCita(entrada);
      ...
      return (
        ...
        (correoLlego
          ? `Ya le llegó el correo con la invitación. `
          : `OJO: el correo de confirmación no salió, así que dile la fecha y la hora en tu ` +
            `respuesta y pídele que la anote. `) +
        `Confírmaselo en una línea, con día y hora.`
      );
    },
  },
