...
export async function enviarConfirmacionCita({ para, nombre, cuando, ics, codigo, cambio }) {
  // Un número de WhatsApp como contacto (cita agendada por ese canal sin
  // correo) no es una dirección: no se intenta, y quien llama ya sabe que la
  // confirmación se la dio el agente en el chat.
  if (!process.env.RESEND_API_KEY || !para || !String(para).includes("@")) return { entregado: false };
...
async function enviarEmailCrudo(asunto, texto, para = destinatarios()) {
  const respuesta = await fetch("https://api.resend.com/emails", {
    signal: AbortSignal.timeout(5_000),
    method: "POST",
    headers: {
...
/**
 * Un correo interno a una dirección concreta, distinta de la de los leads.
 * Lo usa lib/alertas.js para avisarle al OPERADOR (quien mantiene el sistema)
 * sin mezclarlo con la bandeja donde el dueño del negocio recibe sus leads.
 * Lanza si no puede: quien llama decide qué hacer con el fallo.
 */
export async function enviarCorreoInterno({ asunto, texto, para }) {
  if (!process.env.RESEND_API_KEY) throw new Error("sin RESEND_API_KEY");
  const destinos = String(para ?? "")
    .split(",")
    .map((d) => d.trim())
    .filter(Boolean);
  if (destinos.length === 0) throw new Error("sin destinatario");
  await enviarEmailCrudo(asunto, texto, destinos);
}
