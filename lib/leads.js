 ...
 */
export async function enviarConfirmacionCita({ para, nombre, cuando, ics }) {
  if (!process.env.RESEND_API_KEY || !para) return { entregado: false };

  const texto = [
    `Hola${nombre ? ` ${nombre}` : ""},`,
    ``,
    `Tu consultoría gratuita con Intellectum quedó agendada para el ${cuando} (hora de Ecuador).`,
    `Dura 30 minutos y no tiene ningún compromiso.`,
    ``,
    `Adjunto va la invitación para que la agregues a tu calendario.`,
    ``,
    `Si necesitas moverla o cancelarla, respóndenos a este correo o escríbenos al`,
    `WhatsApp +593 98 312 0003.`,
    ``,
  ...
  const respuesta = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [para],
      subject: `Tu consultoría con Intellectum: ${cuando}`,
      text: texto,
      attachments: [
        { filename: "consultoria-intellectum.ics", content: aBase64(ics) },
      ],
    }),
  });
