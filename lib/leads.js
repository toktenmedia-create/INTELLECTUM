...
export async function enviarConfirmacionCita({ para, nombre, cuando, ics, codigo, cambio }) {
  ...
  const texto = [
    ...
    ...(cambio === "cancelada"
      ...
      : cambio === "reagendar"
        ? [
            `El adjunto la quita de tu calendario.`,
            ``,
            `¿La reagendamos? Escríbele a IntelliA en www.intellectum.ec y eliges`,
            `la nueva hora en un minuto, o respóndenos a este correo con la hora`,
            `que mejor te venga.`,
          ]
        : [
    ...
    ``,
    `También puedes responder a este correo o escribirnos al WhatsApp +593 98 312 0003.`,
    ``,
  ...
  const respuesta = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      text: [
        ...
        ``,
        `Intellectum AI Solutions`,
        `info@intellectum.ec · WhatsApp +593 98 312 0003`,
      ].join("\n"),
    }),
  });
