...
export const NEGOCIO = {
  ...
  get nombre() {
    return texto("NEGOCIO_NOMBRE", "Intellectum AI Solutions");
  },
  /** Cómo se le nombra en una frase corriente ("el equipo de X"). */
  get nombreCorto() {
    return texto("NEGOCIO_NOMBRE_CORTO", "Intellectum");
  },
  /**
  ...
  /** Cómo se llama lo que se ofrece gratis al cierre. */
  get cita() {
    return texto("NEGOCIO_CITA", "consultoría gratuita de 30 minutos");
  },
  /**
   ...
   */
  get evento() {
    return texto("NEGOCIO_EVENTO", "consultoría");
  },
  /**
...
/**
 * Lo que le falta a esta copia para atender bien. Se dice una vez al arrancar:
 * un despliegue a medio configurar se descubre mucho mejor en el registro del
 * primer minuto que en el primer correo que sale mudo.
 */
  ...
  if (faltan.length) {
    console.warn(
      `[CLIENTE] la copia de "${CLIENTE}" no tiene ${faltan.join(", ")}: ` +
        "esos datos se omiten en los mensajes en vez de rellenarse con los de Intellectum.",
    );
  }
}
