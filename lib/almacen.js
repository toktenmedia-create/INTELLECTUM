...
async function anexarMensajes({
  ...
  const guardada = await this.guardarConversacion({
    cliente,
    canal,
    sesion,
    nombrePerfil,
  ...
  // EL CONTADOR. Cada respuesta del agente (no las que escribe una persona
  // desde el panel) abre o reutiliza la ventana de esta persona. Contar nunca
  // puede romper el guardado: si falla, se anota y la respuesta ya salió.
  if (nuevos.some((m) => m?.role === "assistant" && m?.via !== "panel")) {
    try {
      await this.contarConversacion({ cliente, canal, sesion, nombrePerfil });
    } catch (err) {
      console.error("[CONTADOR] la conversación no se pudo contar:", err?.message ?? err);
    }
  }
  return guardada;
}
...
/**
 * QUÉ ES UNA CONVERSACIÓN — la unidad que se vende.
 *
 * Regla acordada el 2 de septiembre de 2026: todos los mensajes de una misma
 * persona (su número de WhatsApp o su sesión web) dentro de una ventana de 24
 * horas desde el primer mensaje que el agente respondió son UNA conversación.
 * Si vuelve después de la ventana, es otra. Un mensaje que el agente no
 * respondió no cuenta: no consume el plan del cliente.
 *
 * Cada conversación queda como evento `conversacion` en la bitácora, con el
 * canal y la sesión. Es lo que suma consumoDeConversaciones y lo que el panel
 * muestra contra el tope del plan. Se comparte entre los dos backends.
 */
export const HORAS_POR_CONVERSACION = 24;

async function contarConversacion({
  cliente = CLIENTE,
  canal,
  sesion,
  nombrePerfil = null,
  horas = HORAS_POR_CONVERSACION,
} = {}) {
  if (!canal || !sesion) return { contada: false, motivo: "sin sesión" };
  const abiertas = await this.contarEventos({
    cliente,
    tipo: "conversacion",
    desde: haceHoras(horas),
    sesion,
    tope: 1,
  });
  if (abiertas > 0) return { contada: false, motivo: "ventana abierta" };
  await this.registrarEvento({
    tipo: "conversacion",
    actor: "sistema",
    cliente,
    detalle: { canal, sesion, nombre: nombrePerfil ?? null },
  });
  return { contada: true };
}

/**
 * El primer instante del mes en curso en Ecuador (UTC−5, sin horario de
 * verano), en ISO. El mes de facturación es el del calendario ecuatoriano,
 * no el de los servidores.
 */
export function inicioDelMesEnEcuador(ahora = Date.now()) {
  const enEcuador = new Date(ahora - 5 * 3_600_000);
  return new Date(Date.UTC(enEcuador.getUTCFullYear(), enEcuador.getUTCMonth(), 1, 5, 0, 0)).toISOString();
}

/** Suma las conversaciones contadas por canal, sobre las filas de eventos que le den. */
function resumirConversaciones(filas, { desde, hasta, truncado = false }) {
  const porCanal = {};
  for (const fila of filas) {
    const canal = fila?.detalle?.canal ?? "otro";
    porCanal[canal] = (porCanal[canal] ?? 0) + 1;
  }
  return { desde, hasta: hasta ?? null, total: filas.length, por_canal: porCanal, truncado };
}
...
function crearAlmacenLocal() {
  return {
     ...
     */
    async contarEventos({ cliente = CLIENTE, tipo, desde, ip, sesion, tope = 1000 } = {}) {
    ...
    /** Cuántas conversaciones se contaron en el período (por defecto, el mes en curso). */
    async consumoDeConversaciones({ cliente = CLIENTE, desde = inicioDelMesEnEcuador(), hasta } = {}) {
      const filas = (await leer(ARCHIVOS.eventos)).filter(
        (e) =>
          e.cliente === cliente &&
          e.tipo === "conversacion" &&
          e.creado_en >= desde &&
          (!hasta || e.creado_en < hasta),
      );
      return resumirConversaciones(filas, { desde, hasta });
    },
...
function crearAlmacenSupabase() {
  ...
  return {
    ...
    async respaldo({ cliente = CLIENTE } = {}) {
      ...
      // OJO: en las tablas de datos la columna es cliente_id y guarda el uuid,
      // no el slug. Filtrar por "cliente" devuelve un 400 y tumba el respaldo
      // entero, que además falla en silencio porque quien lo llama lo atrapa.
      const filtro = completo ? "" : `&cliente_id=eq.${await idDe(cliente)}`;
      const foto = {};
    ...
    async listarConversaciones({ cliente = CLIENTE, limite = 50 } = {}) {
      ...
      if (hayColumnaModo !== false) {
        try {
          const filas =
            (await pedir(`${base}&select=canal,sesion,nombre_perfil,mensajes,actualizado_en,modo`)) ?? [];
          hayColumnaModo = true;
      ...
      const filas = (await pedir(`${base}&select=canal,sesion,nombre_perfil,mensajes,actualizado_en`)) ?? [];
    ...
    async contarEventos({ cliente = CLIENTE, tipo, desde, ip, sesion, tope = 1000 } = {}) {
      ...
      const filtros = [
        ...
        sesion ? `detalle->>sesion=eq.${encodeURIComponent(sesion)}` : null,
      ].filter(Boolean);
    ...
    /**
     * Cuántas conversaciones se contaron en el período (por defecto, el mes
     * en curso en Ecuador). Trae solo el detalle, hasta un tope holgado: el
     * plan más grande incluye 1.400 al mes; si se llega al tope se avisa.
     */
    async consumoDeConversaciones({ cliente = CLIENTE, desde = inicioDelMesEnEcuador(), hasta } = {}) {
      const cliente_id = await idDe(cliente);
      const TOPE = 10_000;
      const filas =
        (await pedir(
          `eventos?cliente_id=eq.${cliente_id}&tipo=eq.conversacion` +
            `&creado_en=gte.${encodeURIComponent(desde)}` +
            (hasta ? `&creado_en=lt.${encodeURIComponent(hasta)}` : "") +
            `&select=detalle&limit=${TOPE}`,
        )) ?? [];
      return resumirConversaciones(filas, { desde, hasta, truncado: filas.length >= TOPE });
    },
     ...
     * existe. Un agente que responde con la ficha de casa es un problema; un
     * agente que no responde es uno peor. Si la base NO CONTESTA devuelve
     * { ilegible: true }, que es distinto: ahí no se sabe nada del cliente, y
     * quien llama debe conservar lo que ya tenía en vez de degradar.
     */
    async fichaDeCliente({ cliente = CLIENTE } = {}) {
      try {
        ...
        // NO se devuelve null: null significa "ese cliente no existe", y quien
        // llama lo trata como un alta incompleta. Una base caída no dice eso;
        // no dice nada. La diferencia importa porque de ella cuelga el plan, y
        // tratar una caída como un alta incompleta le apaga al agente la mitad
        // de las herramientas mientras dura la caída.
        return { ilegible: true, ficha: null };
      }
    },
