 ...
 * proyecto equivocado. Los nombres viejos sin sufijo se siguen aceptando
 * porque Vercel todavía los tiene así; cuando se renombren allá, el respaldo
 * y su aviso se quitan de aquí.
 */
let avisadoNombreViejo = false;
export function credencialesSupabase() {
  const url = process.env.SUPABASE_URL_INTELLECTUM || process.env.SUPABASE_URL || "";
  const clave = process.env.SUPABASE_SERVICE_KEY_INTELLECTUM || process.env.SUPABASE_SERVICE_KEY || "";
  if (!avisadoNombreViejo && url && !process.env.SUPABASE_URL_INTELLECTUM) {
    avisadoNombreViejo = true;
    console.warn(
      "[ALMACEN] SUPABASE_URL/SUPABASE_SERVICE_KEY vienen sin sufijo: renómbrelas a " +
        "SUPABASE_URL_INTELLECTUM y SUPABASE_SERVICE_KEY_INTELLECTUM (en .env y en Vercel).",
    );
  }
