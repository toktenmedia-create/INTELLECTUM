...
import { esIntellectum } from "./cliente.js";
 ...
 *
 * Es el MÁS PEQUEÑO a propósito. Antes era el más grande, y con un solo
 * cliente daba igual; con varios, un cliente mal dado de alta —sin plan, o con
 * un plan mal escrito— estrenaba con todas las herramientas abiertas, incluido
 * el cotizador. Que un cliente pida subir de plan es una conversación; que un
 * cliente reciba de más sin pagarlo es un problema que se descubre tarde.
 */
export const PLAN_POR_DEFECTO = "asistente";

/**
 * El plan que se usa cuando la base NO CONTESTA, que no es lo mismo que una
 * ficha sin plan.
 *
 * Una ficha que existe y no dice plan es un cliente mal dado de alta: estrena
 * con el plan más pequeño y alguien lo corrige. Pero si Supabase se cayó o
 * tardó más de cinco segundos, la ficha no dice nada de nadie, y degradar ahí
 * al plan más pequeño le apaga al agente de casa la agenda y el cotizador
 * justo mientras dura la caída: el cliente no ve un error, ve un agente que de
 * pronto no sabe agendar. Ante una lectura fallida se conserva lo que esta
 * copia declara tener, y solo si no declara nada se cae al mínimo.
 */
export function planDeRespaldo() {
  const declarado = (process.env.NEGOCIO_PLAN ?? "").trim();
  if (declarado) return claveDePlan(declarado);
  return esIntellectum() ? "jefe_ventas" : PLAN_POR_DEFECTO;
}
